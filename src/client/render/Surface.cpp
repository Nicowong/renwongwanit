...
void Surface::setSpriteLocation(int i, const Tile& t){
    vertices[i*4+0].position = sf::Vector2f(t.x    , t.y    );
    vertices[i*4+1].position = sf::Vector2f(t.x+t.w, t.y    );
    vertices[i*4+2].position = sf::Vector2f(t.x+t.w, t.y+t.h);
    vertices[i*4+3].position = sf::Vector2f(t.x    , t.y+t.h);
}
